...
| an explicit `LIMIT` above the allowlist size | INTEGRATE | an explicit limit means an eleventh event name could never be truncated into a silently-wrong zero. **Corrected 2026-09-06:** this row said "the default row limit is 100"; measured, the provider appends `LIMIT 101 OFFSET 0` to a query carrying no limit. The report submits `LIMIT 100` against an allowlist of ten, so the margin is unaffected — but the number recorded here was not the provider's |
| resolved `hogql` provenance | INTEGRATE | the API returns its compiled `hogql` rewriting of the submitted text. It is NOT the submitted text and cannot stand in for it: a probe of `SELECT 1` returned `SELECT\n    1\nLIMIT 101\nOFFSET 0`, so accepting it would print a query the report never sent |
| echoed `query` provenance | OPT-OUT | **not available, not declined — this row is a correction.** It previously read that "the API echoes the submitted `query` and the resolved `hogql`" and asserted that INTEGRATE. Measured against the live API on 2026-09-06, with and without the documented `name` parameter: `query` comes back **`null`**. PostHog's published schema declares `query?: string` as optional, so the field is documented but unpopulated on this path. The weakening against Plausible's `query.site_id` echo is therefore one step worse than this file recorded — a response can be bound neither to the project it came from nor to the query it answered. Consequence, carried rather than hidden: `resolveQueryProvenance` accepts an absent echo and marks the provenance `confirmed: false`, and the report states the query on its own authority. Found by UAT gap G-04.2-9, which this row exists to stop a later reader re-deriving |
