...
| Route genuinely owner-owned calls to open questions | Touch privacy, disclosure or data-controller copy (locked — see Copywriting) |
...
Pre-flight measurement found real accessibility defects there — see § Pre-Flight Findings F4/F5/F6
and **OQ-3**, which routes the scope call to the owner rather than silently widening or silently
dropping it.
...
**Scrolling is permitted.** A primary action does not have to be above the fold. `OQ-2` records the
question of whether the owner wants a stricter "reachable without scrolling" reading for any subset;
until answered, the contract is *reachable, visible and hit-targetable after normal scrolling, with
nothing clipped or off-canvas*. Behind a collapsed disclosure that the visitor must open first
...
this phase should consider making to that file's *closed list* (not its machinery), and **OQ-4**.
...
S3 ships **none** — recorded as F5, not invented here. QUAL-02/03 name the HAOO page; a skip link on
the ZPH home is a scope question for the owner (OQ-3), not a silent new requirement.
...
| S3 | **no `main` landmark exists** | recorded as F5, asserted as an observation in the evidence file, not as a pass/fail, pending OQ-3 |
...
anywhere in that repository. That is F6, scoped out pending OQ-3.
...
| Failure threshold | `critical` and `serious` **fail** the run; `moderate` and `minor` are **recorded** in the evidence file with rule id, node target and impact, and do not fail | Default, chosen so the gate is meaningful on day one without a first run that is all noise. **OQ-1** routes tightening to the owner |
...
### F1 — Both `Back to ZERO-PAPER HUB` links point at HAOO itself (in scope, SS-3)
...
Caught by **SS-3's** "names promise destinations truthfully" rule. Fixing it is a two-file change
(source + the test that pins it), and the phase boundary permits it: "where the evidence reveals a
defect, fixing that defect is in scope".

### F2 — `overflow-x-hidden` masks the overflow assertion (in scope, already handled)

`ProductPage.tsx:91` and `ZERO-PAPERHUB/src/App.tsx:207`. Fully addressed by VC-1a/b/c. Named as a
finding because the naive one-line assertion is the obvious thing to write and it would pass
vacuously on both surfaces.

### F3 — `MeasurementDisclosure.tsx` declares focus rings but is not in the closed `FOCUS_SOURCES` list (in scope)
...
The pairing measures **6.30:1** on white, so registering it should be a green change — but that is a
calculation made here, not a test run. Registering it is a one-line addition that closes a real hole
in a closed list. **OQ-4** confirms.

### F4 — `ZERO-PAPERHUB/src/App.tsx` declares focus rings, is not in that repo's `FOCUS_SOURCES`, and registering it would throw (scope: OQ-3)
...
This sits on the ZPH contact form, outside S3. It is not silently widened into scope and not silently
dropped. OQ-3.

### F5 — The ZPH home page has no `<main>` landmark and no skip link (scope: OQ-3)

`grep -c "<main" src/App.tsx` → `0`. A keyboard visitor heading for the Products section traverses
the entire header and hero with no bypass. QUAL-02/QUAL-03 name the HAOO page; S3 is a section of a
page that has neither. Recorded as an observation in the evidence file per SS-2 pending OQ-3.

### F6 — The ZPH home page has no reduced-motion handling (scope: OQ-3)
...
Genuinely owner-owned. Each carries a working default so the phase is not blocked, and each default
is conservative in the direction of *not silently widening scope*.

| ID | Question | Default in force | Why it needs an owner |
|----|----------|------------------|------------------------|
| **OQ-1** | Which axe impact severities block the run? | `critical` + `serious` block; `moderate` + `minor` recorded | Blocking on all four is the stricter, more honest bar, but the first run's noise level is unknown. Tightening later is cheap; loosening after publishing a pass is not |
| **OQ-2** | Does "no hidden primary actions" (QUAL-01) mean *visible without scrolling* for any subset of § Primary Actions? | Reachable, visible and hit-targetable after normal scrolling, with nothing clipped or off-canvas | An above-the-fold requirement is a product decision about the funnel, not an accessibility threshold. No shipped surface currently guarantees it at 360 px |
| **OQ-3** | Does the ZPH-side evidence stop at the Products section (S3), or cover the whole home page a visitor traverses to reach it? | Stops at S3. F4/F5/F6 are recorded as observations, not asserted as failures | F4/F5/F6 are real defects on a live public page. Asserting them expands a proving phase into a remediation phase in a repository D-03 says receives code commits only when evidence forces it |
| **OQ-4** | Register `MeasurementDisclosure.tsx` in HAOO's `FOCUS_SOURCES` (F3)? | Yes — recommended, one line, expected green | Touching a ratified closed list is exactly the kind of change 04.2 required a recorded reason for |

**Not an open question, and named so it is not mistaken for one:** the measurement/PostHog
...
| error | ZPH home contact form (outside S3) | ⚠ unresolved | Has its own submit, status region and honeypot, and a `focus:ring-green-400` indicator computing ≈1.74:1 (F4). Scope pending OQ-3 |
